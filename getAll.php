...
if ($dicom == '/dicom.php') {
    ...
    $kondisi = "WHERE (xray_workload.status = 'waiting' AND xray_order.dokradid = '$dokradid' AND study.study_datetime >= '2026-01-31')
                OR (xray_workload.status = 'waiting' AND xray_order.dokradid IS NULL)
                AND study.study_datetime >= '2026-01-31'
...
while ($row = mysqli_fetch_array($query)) {
    ...
    $data[] = [
        "no" => '<div style="text-align : center;">' . $i . '</div>',
