...
function pdfProsesExpertise($uid, $pdf)
{
    ...
    $patientid = substr(defaultValue($row['patientid']), 0, 29);
    ...
    $nip = $row_dokrad['nip'];
    $link_dokrad_img = "http://127.0.0.1:8000/storage/" . $row_dokrad['dokrad_img'];
    ...
    $link_surat_image = "http://127.0.0.1:8000/storage/" . $kopSurat['image'];
    // kondisi ketika gambar kop surat null & ketika server laravel error
    $kop_surat_image = $kopSurat['image'] == null ? 'kop_wonosari1.png' : (@file_get_contents($link_surat_image) === false ? 'kop_wonosari1.png' : $link_surat_image);
    ...
    $pdf->image($kop_surat_image, 6, 1, 198);
    $pdf->MultiCell(0, 1, ' 
    ...
    $pdf->Cell(3, 5, ':', 0, 0, 'L');
    $pdf->Cell(65, 5, defaultValue($name_dep), 0, 1, 'L');
    // -----------------
    ...
    $pdf->Cell(3, 5, ':', 0, 0, 'L');
    $pdf->Cell(65, 5, defaultValue($dokrad_name), 0, 1, 'L');
    // -----------------
    $pdf->Cell(28, 5, 'Tgl Lahir / Umur', 0, 0, 'L');
    $pdf->Cell(3, 5, ':', 0, 0, 'L');
    $pdf->Cell(55, 5, defaultValueDate($pat_birthdate) . ' ' . $age, 0, 0, 'L');
    // ------------------
    $pdf->Cell(35, 5, 'Waktu Pemeriksaan', 0, 0, 'L');
    $pdf->Cell(3, 5, ':', 0, 0, 'L');
    $pdf->Cell(65, 5, defaultValueDateTime($study_datetime), 0, 1, 'L');
    // -----------------
    ...
    $pdf->Cell(3, 5, ':', 0, 0, 'L');
    $pdf->Cell(65, 5, defaultValue($study_desc_one) . ' ' . defaultValue($study_desc_two), 0, 1, 'L');
    // -----------------
    $pdf->Cell(28, 5, 'Klinis', 0, 0, 'L');
    $pdf->Cell(3, 5, ':', 0, 0, 'L');
    $pdf->Cell(158, 5, trim($spc_needs_one) . ' ' . trim($spc_needs_two), 0, 2, 'L');
    ...
    if ($expertise['signature_dokter_radiologi'] == 'qr_code') {
        ...
        $pdf->image($dokrad_img, 135, $pdf->GetY(), 50);
    ...
    foreach ($series_iuids as $index => $series_iuid) {
        ...
        while ($row_series_instance = mysqli_fetch_assoc($series_instance)) {
            try {
                ...
                $link_dicom_jpg = "http://192.168.133.9:9090/dcm4chee-arc/aets/DCM4CHEE/wado?requestType=WADO&studyUID=$uid&seriesUID=$series_iuid&objectUID=$sop_iuid";
